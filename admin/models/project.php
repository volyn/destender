...
class ProjectsModelProject extends JModelAdmin {
	...
	public function getForm( $data = array( ), $loadData = true ) {
		...
		if ( !$user->authorise( 'core.edit.state', '#__projects.' . $this->getState( 'extdataedit.id' ) ) ) {
	...
	protected function loadFormData() {
		$data = JFactory::getApplication()->getUserState( 'com_projects.edit.project.data', array() );
		if ( empty( $data ) ) {
			$data = $this->getItem();
		}
		return $data;
	...
	{
		if ( !empty( $record->id ) ) {
			return JFactory::getUser()->authorise( 'core.delete', '#__projects.' . (int)$record->id );
		}
	}
	...
	protected function canEditState( $record )
	{
		...
		if ( !empty( $record->id ) ) {
			return $user->authorise( 'core.edit.state', '#__projects.' . (int)$record->id );
		} // New article, so check against the category.
		elseif ( !empty( $record->catid ) ) {
			return $user->authorise( 'core.edit.state', '#__projects.' . (int)$record->catid );
