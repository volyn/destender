...
if ( $saveOrder ) {
	$saveOrderingUrl = 'index.php?option=com_projects&task=projects.saveOrderAjax&tmpl=component';
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="articleList">
				...
				<?php foreach ( $this->items as $i => $item ) :
					...
					$canEdit = $user->authorise( 'core.edit', '#__projects.' . $item->id );
					$canCheckin = $user->authorise( 'core.manage', 'com_projects' ) || $item->checked_out == $userId || $item->checked_out == 0;
					$canEditOwn = $user->authorise( 'core.edit.own', '#__projects.' . $item->id ) && $item->created_by == $userId;
					$canChange = $user->authorise( 'core.edit.state', '#__projects.' . $item->id ) && $canCheckin;
					?>
						...
						<td class="nowrap has-context">
							<div class="pull-left">
								<?php if ( $canEdit || $canEditOwn ) : ?>
									<a href="<?php echo JRoute::_( 'index.php?option=com_projects&task=project.edit&id=' . $item->id ); ?>" title="<?php echo JText::_( 'JACTION_EDIT' ); ?>">
